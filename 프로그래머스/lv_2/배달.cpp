...
/*   Updated: 2022/08/07 12:32:27 by minskim2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int solution(int n, vector<vector<int>> road, int k) {
	...
	for (int i=0; i<road.size(); ++i) { // 도로 검사
		if (road[i][0] == 1 && road[i][2] <= k && visit_cost[road[i][1]] > road[i][2]) {
			q.push_back(road[i][1]);
			visit_cost[road[i][1]] = road[i][2];
		}  // 1번 마을에서 갈 수있는 도로의 경우 기존의 방문 비용과 비교해서 재설정
		else if (road[i][1] == 1 && road[i][2] <= k && visit_cost[road[i][0]] > road[i][2]) {
			q.push_back(road[i][0]);
			visit_cost[road[i][0]] = road[i][2];
		}  // 위와 같은데 1번 마을이 road[1]인 경우
	}
